...
from app.domains.enrich.hourly.synthesis import (
    build_event_briefing_materials,
    build_hourly_briefing_digest,
...
async def generate_previous_hour_digest() -> None:
    ...
    max_pick = min(12, limits["max_candidates"])
    try:
        entries = ctx["entries"]
        ...
        try:
            materials = build_event_briefing_materials(event_items)
        ...
        if not is_valid_digest_format(body):
            body = build_hourly_briefing_digest(
                ctx["title"],
                event_items,
                reason=degrade_reason or "模型输出未通过快报格式校验，已回退为规则快报。",
            )
