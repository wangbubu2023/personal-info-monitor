...
it?"* by persisting a small circuit-breaker record under
...
Per the enhancement plan (§4.3) we deliberately keep this in JSON metadata
rather than a dedicated table for the MVP. The scheduler reads
``cooldown_until`` to skip automatic fetches without disabling the source
outright, while manual triggers bypass the cooldown entirely.
"""
...
def record_fetch_failure(
    ...
    *,
    ...
    if cooldown_seconds and cooldown_seconds > 0:
        ...
        record["cooldown_until"] = (now + timedelta(seconds=scaled)).isoformat() + "Z"
