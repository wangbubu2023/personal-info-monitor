"""Fetch coordinator knitting collection and ingest stages together.
...
def _commit_and_dispatch_session_alert(db: Session, source: Source) -> None:
...
def _apply_keyword_filter(db: Session, source: Source, content_objects: list[Content]) -> tuple[list[Content], int]:
    ...
    if not keywords:
        ...
        warning = "keyword_filter_misconfigured"
        if warning not in warnings:
            warnings.append(warning)
        metadata["warnings"] = warnings
        metadata["keyword_filter_warning"] = {
            "code": warning,
            "message": "Keyword filter is enabled but no enabled keywords exist; fetch was left unfiltered.",
        }
        source.metadata_ = metadata
        logger.warning("Source %s has keyword filter enabled but no keywords configured; keeping all items", source.name)
...
async def run_fetch_pipeline(db: Session, source: Source, manual_trigger: bool = False) -> Dict[str, Any]:
    ...
    def _content_fulltext_stats(contents: list[Content]) -> tuple[int, int]:
        ...
        for content in contents:
            text = (content.full_content or "").strip()
            metadata = content.metadata_ if isinstance(content.metadata_, dict) else {}
            if text or metadata.get("fulltext_status") or metadata.get("article_fulltext"):
                fulltext_total += 1
                if metadata.get("article_fulltext") or metadata.get("fulltext_status") == "full" or len(text) >= 280:
                    fulltext_ok += 1
        return fulltext_ok, fulltext_total

    def _preferred_strategy() -> str | None:
        diag = getattr(source, "_runtime_fetch_diag", None)
        if isinstance(diag, dict) and int(diag.get("hydrated") or 0) > 0:
            return "article_hydration"
        ...
        if metadata.get("rss_url") or metadata.get("rss_urls"):
            return "rss"
        return str(source_type).lower() or None
