"""HTML → main-content text extraction for ingest.

Used by the build-content stage to turn hydrated article HTML into plain
text we can store in ``Content.full_content``. Tries three strategies in
order — readability-lxml + markdownify, trafilatura, BeautifulSoup —
falling back to thinner extractors when the previous one produced too
little signal. Also exposes :meth:`ContentExtractor.extract_metadata`
which the Reader / website collector use for OG-title and author hints.

Moved out of ``app.processors.extractor`` as part of Phase 3 step 4 of
the module-refactor blueprint. The old module path remains as a
re-export shim through Phase 7 so existing
``unittest.mock.patch("app.processors.extractor.ContentExtractor", ...)``
test targets continue to resolve and external callers
(``processors/content_processor.py``, ``services/reader/body_loader.py``,
``api/contents_reader.py``) keep working.
"""
