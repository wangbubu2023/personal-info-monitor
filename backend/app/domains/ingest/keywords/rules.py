...
Moved out of ``app.services.keyword_rules`` as part of Phase 3 step 4 of
the module-refactor blueprint. The old module path remains as a
re-export shim through Phase 7.

The bilingual expansion in :func:`build_equivalent_terms` is technically
an *enrich* concern (it calls the LLM-powered :class:`Translator`), but
the call is lazy-imported inside the function — at module-import time
the ingest domain stays free of any ``processors.translator`` reference,
keeping the Phase 3 boundary check (``ingest → llm/translator``
forbidden) clean.
"""
