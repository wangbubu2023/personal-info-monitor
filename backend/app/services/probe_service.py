...
:mod:`app.services.probe_strategies.registry`. ``ProbeService`` no longer
...
from app.services.probe_strategies.registry import STRATEGY_REGISTRY
from app.services.probe_strategies.result import ProbeResult
from app.services.probe_strategies.rss import (  # re-exported for callers
    KNOWN_RSS_FEEDS,
    _UNFETCHABLE,
    _USE_SCRAPING,
)
...
class ProbeService:
    ...
    def _build_cookie_jar(scope_url: str) -> Optional[aiohttp.CookieJar]:
        ...
        except Exception as exc:  # yarl raises varied exceptions on malformed URLs
