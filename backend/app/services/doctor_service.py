...
class DoctorService:
    ...
    def _audit_database(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check migrations (simple check if alembic table exists)
            has_alembic = self.db.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='alembic_version'")).scalar()
    ...
    def _audit_environment(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "is_low_space": free < (1024 * 1024 * 1024) # 1GB
            }
        except Exception as e:
    ...
    def _audit_collectors(self) -> Dict[str, Any]:
        ...
        # Also probe patchright (the stealth fork) so doctor surfaces which
        # backend will actually launch. If missing, Datadome / Cloudflare
        # class sites (NYT, WSJ, Bloomberg…) will fall back to vanilla
        # playwright and almost certainly fail the paywall.
        ...
        if master_on:
            # Heads-up that we still expose a browser automation surface by
            # default — operators on hardened hosts may want to flip
            # PIM_FEATURE_PLAYWRIGHT=false to eliminate it entirely.
            warnings.append(
        ...
        payload: Dict[str, Any] = {
            ...
            "chrome_found": True,  # simplified, keeps parity with pre-refactor shape
