...
const AtomMiniList: React.FC<AtomMiniListProps> = ({ atomIds, limit = 12 }) => {
  ...
  if (loading) return <p className="text-xs text-[#536b82]">加载关联原子…</p>
  ...
  return (
    <div className="space-y-2">
      {atoms.map((atom) => (
        <div
          key={atom.atom_id}
          className="rounded-xl border border-[#d8e4ec] bg-[#f8fbfc] px-3 py-2"
        >
          <div className="flex flex-wrap items-center gap-2 text-[11px] text-[#536b82]">
            ...
            <Link to={`/reader/${atom.content_id}`} className="ml-auto font-medium text-[#0b6f91] underline">
              Reader
            </Link>
          </div>
          <p className="mt-1 text-xs text-[#22324a]">{atomSummary(atom)}</p>
        </div>
      ))}
      {atomIds.length > limit && (
        <p className="text-[11px] text-[#536b82]">仅显示前 {limit} 条，共 {atomIds.length} 条。</p>
      )}
    </div>
  )
}
